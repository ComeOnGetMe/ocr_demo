...
def train(localsift):
    ...
    kp, des = localsift.detectAndCompute(tr_img, None)
...
def trim(str, length):
    for i in xrange(len(str)):
        if (i +length) < len(str) and ' ' not in str[i:i + length]:
...
def orientation(imagebytes, log = True):
    ...
    try:
        decodestr = base64.decodestring(imagebytes)
        imgarr = np.fromstring(decodestr, np.uint8) 
    except:
        print "Orientation: Illegal file format." 
...
def extract(pdf_bytes, verbose = True, threshold = 110, delta = 10, log = True):
    ...
    for keys in dataform:
        ...
        if log:
            a = open('log.txt', 'w')
        for y in xrange(top - delta * 2, top, 2):
            cropimg = img[y:y + height, left:left + width]
            # tmp = img[top:top + height, left:left + width]
            # cropimg = cv2.resize(tmp, (0,0), 1.5, 1.5, cv2.INTER_AREA)
